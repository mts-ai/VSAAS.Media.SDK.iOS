...
/**
 * Custom
 * Callback for notify external world about something.
 */
typedef struct AVIONotifyCB {
	int(*callback)(void* opaque, AVDictionary *notify);
	void *opaque;
} AVIONotifyCB;
...
/**
 * Custom
 * Same as previous with notify callback
 */
int avio_open2_with_notify(AVIOContext **s, const char *url, int flags,
	const AVIOInterruptCB *int_cb, const AVIONotifyCB *notify_cb, AVDictionary **options);
